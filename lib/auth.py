...
class GAuth(object):
	...
	def __g_token(self):
		...
		return json.loads(response.content.replace(b")]}'", b""))[0][0][2]
	...
	def Glogin(self):
		...
		if b"CheckCookie" in response.content:
			return ({"status":1,"session":self.session})

		if b"INCORRECT_ANSWER_ENTERED" in response.content or b"LOGIN_CHALLENGE" in response.content:
